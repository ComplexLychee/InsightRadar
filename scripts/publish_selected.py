...
def generate_post(papers_data, range_display, start_date, end_date):
    ...
    if range_display == "过去1周":
        ...
        title = f"Insight Radar · {range_display}精选 ({start_date} ~ {end_date})"
        tag_label = "论文洞察"
...
> 📅 **检索范围**：{range_display} ({start_date} ~ {end_date})  
...
def main():
    ...
    start_date = range_match.group(1) if range_match else basename
    end_date = range_match.group(2) if range_match else basename
    ...
    filename = f"_posts/{end_date}-insight-radar-{range_slug}.md"
